...
| `llama-3.3-70b` | 7 | 4.7% | 偶发 padded 1,极端情况下 padded 9 |
| `gpt-4o-mini` | **27** | **18%** | 全部为 LLM under-output,常见 padded 1、5、6 |

GPT-4o-mini 存在稳定的 under-output 现象:在对 10 项进行 listwise rerank 时,它经常只输出前 1–6 个结果,后续项目由 parser 按原顺序补齐。Claude 几乎没有此问题;Gemini 切换到 `2.5-flash-lite` 后,API fail 降为 0。
...
- **4 个 reranker LLM 不是完全等价的 baseline**:`gpt-4o-mini` 在 18% 的 rows 上对 10 项 listwise rerank 只输出前几项,后续由 parser 补齐。严格来说,它与其他 LLM 不能进行逐 row 的完全等价比较。CSV 中的 `reranker_padded_*` 列可用于剔除受影响 rows 后进行更严格的分析。
